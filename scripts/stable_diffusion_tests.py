...
def init_latent_diffusion_from_mode(mode):
    ...
    if mode == 4:
        with section(
                "to initialize latent diffusion empty and without loading weights, with external initialization function, then load its submodels from disk"):
    ...
    if mode == 5:
        with section(
                "to initialize latent diffusion empty and without loading weights, then load its submodels from disk"):
            device = get_device(0)
    ...
    if mode == 6:
        with section("to load latent diffusion saved model"):
            ...
            unet_model.eval()
            latent_diffusion_model = initialize_latent_diffusion(path=None, autoencoder=autoencoder,
                                                                 unet_model=unet_model,
                                                                 clip_text_embedder=clip_text_embedder,
    ...
    if mode == 8:
        with section(
                "to initialize latent diffusion empty and without weights, then loading the submodels tree from disk"):
            device = get_device(0)
            latent_diffusion_model = LatentDiffusion(linear_start=0.00085,
                                                     linear_end=0.0120,
                                                     n_steps=1000,
...
def generate_images_from_dist(
    ...
    with torch.no_grad():
        with tqdm(total=total_images, desc='Generating images', ) as pbar:
            ...
            noise_fn = lambda shape, device=None: get_torch_distribution_from_name(DIST_NAME)(**params).sample(
                shape).to(device)
            grid_rows = []
            img_counter = 0
...
def generate_images_from_dist_dict(
    ...
    for distribution_index, (distribution_name, params) in enumerate(distributions.items()):
        grid = generate_images_from_dist(txt2img, (distribution_name, params), prompt_prefix=prompt_prefix,
                                         artist_file=artist_file, num_artists=num_artists, batch_size=batch_size,
                                         temperature=temperature)
        img_grids.append(grid)

    dest_path = os.path.join(TESTS_OUTPUT_DIR,
                             f"grid_all_{DIST_NAME}{VAR_RANGE[0].item():.2f}_{VAR_RANGE[-1].item():.2f}_{VAE_INIT_MODE}{CLIP_INIT_MODE}{LATENT_DIFFUSION_INIT_MODE}.jpg")
...
def generate_images_from_temp_range(
    ...
    img_grids = []
    for distribution_index, (distribution_name, params) in enumerate(distributions.items()):
        noise_fn = lambda shape, device=None: get_torch_distribution_from_name(DIST_NAME)(**params).sample(shape).to(
            device)
        img_rows = []
    ...
    dest_path = os.path.join(output_dir, "grid_all.jpg")
...
def main():
    ...
    generate_images_from_dist_dict(DISTRIBUTIONS, txt2img, num_artists=NUM_ARTISTS, batch_size=1,
                                   temperature=TEMPERATURE)
