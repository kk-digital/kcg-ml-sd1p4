...
parser.add_argument('-p', '--embedded_prompts_dir', type = str, default = EMBEDDED_PROMPTS_DIR,\
...
parser.add_argument('-od', '--output_dir', type = str, default = OUTPUT_DIR, help = "The output directory. defaults to OUTPUT_DIR constant, which should be './output/noise-tests/from_embeddings'")
...
def init_stable_diffusion(device, sampler_name = "ddim", n_steps = "20", ddim_eta = 0.0):
    ...
    stable_diffusion = StableDiffusion(device = device, sampler_name=sampler_name, n_steps=n_steps, ddim_eta=ddim_eta)
...
def generate_images_from_embeddings(
    ...
    embeddings = torch.load(join(embeddings_dir, 'embedded_prompts.pt'), map_location=DEVICE)

    for embedding in embeddings:
        print(embedding.shape)
    stable_diffusion = init_stable_diffusion(device=cuda_device, sampler_name=sampler_name, n_steps=n_steps, ddim_eta=ddim_eta)
        ...
        with tqdm(total=total_images, desc='Generating images', ) as pbar:
            ...
            for prompt_index, prompt in enumerate(embeddings):
                ...
                for seed_index, noise_seed in enumerate(noise_seeds):
                    p_bar_description = f"Generating image {img_count+1} of {total_images}"
            ...
            save_image_grid(torch.cat(img_grid, dim=0), join(output_dir, "grid.jpg"), nrow = NUM_SEEDS)
def main():
